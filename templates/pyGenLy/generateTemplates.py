...
def transposePitchSwitch(inputPitchList):
    """returns the list of corresponding transposition pitch:
    """
    mirrorDic = {
        'd': 'bes\'', # soprano sax, clarinet
        'bes\'': 'd', # soprano sax, clarinet
        'd\'': 'bes', # soprano sax, clarinet
        'd\'\'': 'bes,', # tenor sax
        'd\'\'\'': 'bes,,', # trombone
        'bes': 'd\'', # soprano sax, clarinet
    ...
    return [mirrorDic[k] for k in inputPitchList]
...
def generateCommonScoreLy(targetPitchDic, midiInstrumentDic, clefDic,
    ...
    originPitchStr = '\n'.join([
        'originPitch%s = %s' %(
            label, transposePitchSwitch([targetPitch,])[0])
        for label, targetPitch in targetPitchDic.items()])
    midiInstrStr = '\n'.join([
...
def generateNotes(instrument, optLabel=''):
    ...
    notesStr = ('%s%sNotes = \\transpose \\originPitch \\targetPitch%s {\n' %(
        optLabel, instrument, instrument) +
                ...
                '      \\transposition \\originPitch%s %% for MIDI files?' %(
                    instrument)+
                ' to be verified\n' +
                '\n'+
                '      \\clef \\clef%s\n' %instrument +
